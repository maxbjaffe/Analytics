...
if uploaded_file is not None:
    ...
    # Show custom metrics for each report type
    ...
    if report_type == "TVQI Report":
        ...
        st.metric("Impressions", f"{int(impressions):,}")
        st.metric("Completed Views", f"{int(completed_views):,}")
        ...
        st.metric("Impressions", f"{int(impressions):,}")
        st.metric("Total Cost", f"${cost:,.2f}")
        st.metric("Saved Impressions", f"{int(savings_impressions):,}")
        st.metric("Saved Cost", f"${savings_cost:,.2f}")
        ...
        st.metric("Total Impressions", f"{int(impressions):,}")
        st.metric("Total Cost", f"${cost:,.2f}")
        st.metric("Unique Households", f"{int(unique_hh):,}")
        st.metric("Avg Frequency/HH", f"{frequency_hh:.2f}")
